...
describe('NPS', function() {
  ...
  describe('#score()', function() {
    ...
    describe('test NPS accuracy with a distribution argument', function () {
      it('should return +1.0 with a distribution of all promoters', function () {
        assert.equal(nps.score(
          {detractors: 0, neutrals: 0, promoters: 650}
        ), 1);
      });
      it('should return    0 with a distribution of all neutrals', function () {
        assert.equal(nps.score(
          {detractors: 0, neutrals: 246, promoters: 0}
        ), 0);
      });

      it('should return -1.0 with a distribution of all detractors', function () {
        assert.equal(nps.score(
          {detractors: 123, neutrals: 0, promoters: 0}
        ), -1);
      });
      it('should return    0 with sum(promoters) == sum(detractors)', function () {
        assert.equal(nps.score(
          {detractors: 650, neutrals: 0, promoters: 650}
        ), 0);
      });
      it('should return -0.3 for the given dataset(s)', function () {
        assert.equal(nps.score(
          {detractors: 10, neutrals: 6, promoters: 4}
        ), -0.3);
      });
      it('should return +0.4 for the given dataset(s)', function () {
        assert.equal(nps.score(
          {detractors: 2, neutrals: 8, promoters: 10}
        ), +0.4);
      });
      it('should return +0.6 for the given dataset(s)', function () {
        assert.equal(nps.score(
          {detractors: 2, neutrals: 0, promoters: 8}
        ), +0.6);
      });
    });
    ...
    describe('test NPS accuracy with an array of distributions', function () {
      it('should return -0.3 for the given dataset(s)', function () {
        assert.equal(nps.score([
          {detractors: 2, neutrals: 0, promoters: 3},
          {detractors: 4, neutrals: 3, promoters: 1},
          {detractors: 4, neutrals: 3, promoters: 0}
        ]), -0.3);
      });
      it('should return +0.4 for the given dataset(s)', function () {
        assert.equal(nps.score([
          {detractors: 1, neutrals: 3, promoters: 3},
          {detractors: 1, neutrals: 3, promoters: 6},
          {detractors: 0, neutrals: 2, promoters: 1}
        ]), +0.4);
      });
      it('should return +0.6 for the given dataset(s)', function () {
        assert.equal(nps.score([
          {detractors: 2, neutrals: 0, promoters: 1},
          {detractors: 0, neutrals: 0, promoters: 1},
          {detractors: 0, neutrals: 0, promoters: 6}
        ]), +0.6);
      });
    });
  });
  ...
  describe('#stats()', function() {
    ...
    describe('test variance with a distribution argument', function () {

      // stats.stackexchange.com/questions/18603/how-can-i-calculate-margin-of-error-in-a-nps-net-promoter-score-result
      it('should return 5/9 for the NPS variance of the given dataset(s)', function () {
        var dist = { detractors: 54, neutrals: 108, promoters: 162 };
        assert.equal(nps.stats(dist).variance, 5/9);
      });
      it('should return sqrt(5/9) for the NPS stddev of the given dataset(s)', function () {
        var dist = { detractors: 54, neutrals: 108, promoters: 162 };
        assert.equal(nps.stats(dist).stddev, Math.sqrt(5/9));
      });
      it('should return sqrt(5/9)/(sqrt(324)) for the NPS stdderr of the given dataset(s)', function () {
        var dist = { detractors: 54, neutrals: 108, promoters: 162 };
        assert.equal(nps.stats(dist).stderr, Math.sqrt(5/9)/Math.sqrt(324));
      });
    });
  });
